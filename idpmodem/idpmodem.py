...
class Modem(object):
    ...
    while self.serial_port.isOpen() and not self._terminate:
      if len(self.at_commands_pending) > 0:
        if self.at_command_active is None:
          ...
          at_cmd = self.at_commands_pending[0] if len(self.at_commands_pending) > 0 else None
  ...
  def _update_stats_at_response(self, response):
    ...
    if 'AT%GPS' in response.command.upper():
      ...
      self.log.debug(
        "S-register query {} processed".format(response.command[4:].replace('?', '')))
    ...
    if valid_response:
      ...
      for c in reg_config:
        ...
        if value != reg.default:
          self.log.warning("{} value {} does not match target {}".format(
            name, value, reg.default))
          reg.set(value)
    self._init_modem(
  ...
  def _cb_get_volatile_sreg(self, valid_response, responses, request):
    if valid_response:
      ...
      self.log.error(
        "Failed to query volatile S-registers - {}".format(responses))
    ...
    if isinstance(mo_message, MobileOriginatedMessage):
      ...
      self.submit_at_command(at_command='AT%MGRT={name},{priority},{sin}{min},{data_format},{data}'
                   .format(name='\"{}\"'.format(p_msg.q_name),
                       ...
                       sin=mo_message.sin,
                       min='.{}'.format(
                         mo_message.min) if msg_min is not None else '',
  ...
  def get_location(self, callback, name=None, fix_age=30, nmea=['RMC', 'GGA', 'GSA', 'GSV']):
    ...
    if self.location_pending is None:
      ...
      self.log.warning(
        "Another location request is pending, try again later")
