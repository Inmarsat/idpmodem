    ...
    if msg_sin == 255:
        if msg_min == 1:
            ...
            if len(msg_content) > 6 and str(msg_content[6:]) in motes:
                    lora_mac = str(msg_content[6:])
            else:
                if len(msg_content) > 6:
                    log.warning("Invalid LoRa MAC requested (%s) - using %s", str(msg_content[6:]), motes[0])
                else:
                    log.warning("No LoRa MAC requested - using %s", motes[0])
                lora_mac = motes[0]

            new_lora_interval = int(msg_content[2:6], 16)
            if 30 <= new_lora_interval <= 300:
                log.info("Changing LoRa tracking interval to %d on mote %s", new_lora_interval, lora_mac)
                mqtt_down_lora(lora_client, mac_node=lora_mac, lora_payload=new_lora_interval)
            else:
                log.warning("Invalid LoRa interval %d (must be 30-300 seconds)", new_lora_interval)
                # TODO: send an error response indicating 'invalid value' over the air
        else:
            log.warning("Unsupported MIN=" + str(msg_min))
            # TODO: send an error response indicating 'invalid command' over the air
    else:
...
def on_lora_message(client, userdata, msg):
    ...
    if lora_mac not in motes:
        log.info("New LoRa mote found: " + str(mac_str))
        motes.append(lora_mac)
        # TODO: optimize using a byte array
...
def mqtt_down_lora(client, mac_node, lora_payload):
    ...
    if type(lora_payload) == int:
        # TODO: add scalability beyond short int
    ...
    data_to_mDot = "{ \"data\": \"" + b64_payload + "\" }"
    log.info("Sending MQTT Topic: " + topic_down + " Data: " + data_to_mDot)    # TODO: change to debug
...
def main():     # TODO: trim more functions out of main, refactor for module import to run as thread
    ...
    lora_client.on_connect = on_lora_connect
    lora_client.on_message = on_lora_message
    motes = []
    ...
    if sys.platform.lower().startswith('linux2'):
        ...
        lora_client.connect("127.0.0.1", 1883, 60)
    ...
    try:
        ...
        if ser.isOpen():
            ...
            ever_connected = init_com(max_attempts=3)
